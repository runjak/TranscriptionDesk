...
Am Schluss des Berichtes wird eine Zusammenfassung der ,,lessons learned'' sowie ein Ausblick auf mögliche die weitere Entwicklung des Projektes gegeben.
...
Deswegen haben wir für unsere anfängliche Planung ein agiles Entwicklungsmodell verwendet.\\
...
in dem konkrete Bugs gelöst werden sollen und, falls dann noch ausreichend Zeit bleibt, direkt weitere Funktionen implementiert werden können. Diese Meilensteine wurden im Projektverlauf konkretisiert.
\subsection{Struktur der Datenbasis}
Die Scans, die den Bedarf an einer Transkriptionssoftware weckten,
...
Deswegen wurde zu diesem Zweck vorerst nur die oben genannte Funktionalität implementiert.\\
...
Diese Aufzählung steckt weitestgehend unsere Vorstellungen zur Nutzerbasis ab, wobei es durchaus interessant wäre, in einem späteren Stadium eine Umfrage zu starten, um zu sehen, welchen Bezug die Nutzer zu unserem Projekt haben. Mit diesem Wissen könnten eventuell weitere Anreize eingebracht werden.
...
\begin{description}
\item[\textbf{/LF100/ Geschäftsprozess:}] Erstellen der Webseite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Jeder Benutzer sollte auf die Webseite gelangen können und die Möglichkeit besitzen,
diese zu betrachten und via der Navigationsleiste auf andere Inhalte der Webseite zuzugreifen.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF110/ Geschäftsprozess:}] Erstellen der Willkommensseite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Wenn ein Benutzer auf die Webseite zugreift,
so gelangt er zunächst auf dieser Seite. Die Schaltfläche mit dem Kennzeichen ,,TranscriptionDesk'' in der Navigierungsleiste verweist auf diese Seite.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF120/ Geschäftsprozess:}] Erstellen der ,,About''-Seite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Diese Seite kann via der Navigationsleiste aufgerufen werden, um eine genauere Erläuterung des Projektes zu erhalten.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF130/ Geschäftsprozess:}] Erstellen der Statistikseite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Durch die Navigationsleiste kann der Benutzer allgemeine Statistiken des Projektes
sowie die Bestenlisten aller Zeit oder des Monats näher betrachten. Die Bestenlisten werden bzgl. \textbf{/NF171/} angezeigt und aktualisiert.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF140/ Geschäftsprozess:}] Erstellen der Kontaktseite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Via der Schaltfläche ,,Contact'' gelangt der Nutzer auf die Kontaktseite, welche das Impressum des Projektes beinhaltet
sowie allgemeine Kontaktinformationen des Lehrstuhls.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF150/ Geschäftsprozess:}] Erstellen der Scansseite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Sofern ein Benutzer nicht angemeldet ist, erhält dieser die Aussage ,,Sorry, we've got nothing to display for you here.
It may well be that we can display something if you decide to login.''
Ist der Benutzer angemeldet so gelangt er zu einer Tabellenauflistung aller vorhandenen Scans und kann sich ein Element via des ,,identifier'' aufrufen.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF151/ Geschäftsprozess:}] Erstellen der Seite für einzelne Elemente
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Nachdem der Benutzer eines der Elemente aus der Tabelle auf der Scansseite auswählte,
werden nun alle einzelnen Bilder des Elementes angezeigt. Der Benutzer kann auf jedes dieser Bilder klicken, um zu der Transkriptionsansicht des Bildes zu gelangen.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF152/ Geschäftsprozess:}] Erstellen der Transkriptionsseite für Objekte
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Auf dieser Seite kann der Nutzer das von ihm ausgewählte Objekt komplett oder partiell transkribieren.
Das Objekt wird nach \textbf{/LF300/} dargestellt, wobei der Nutzer die ausgewählten Bereiche nach \textbf{/LF400/} transkribiert.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF160/ Geschäftsprozess:}] Erstellen der Transkriptionsseiten
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Sollte der Benutzer auf die Schaltfläche ,,Transcribing'' klicken,
so erscheint eine Dropdown-Liste mit drei verschiedenen Einträgen wobei der Benutzer entweder eines davon auswählen oder die Liste wieder schließen kann.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF161/ Geschäftsprozess:}] Erstellen der Erklärungsseite für Transkriptionen
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Sofern der Benutzer ,,What are transcriptions?'' ausgewählt hat,
gelangt dieser auf eine Seite, welche ihm eine grobe Erläuterung gibt, was Transkriptionen sind und wie sie erstellt werden sollten.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF162/ Geschäftsprozess:}] Erstellen des Tutorials
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Hat der Benutzer den Punkt ,,Tutorial'' ausgewählt,
so gelangt dieser auf eine Seite mit einer detaillierten Erklärung, wie man einzelne Objekte transkribieren kann und andere Tipps.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF163/ Geschäftsprozess:}] Erstellen der Seite mit transkribierten Objekten
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Schlussendlich, wenn der Benutzer ,,Transcribed documents'' auswählt,
kann dieser verschiedene Objekte betrachten, welche schon voll-ständig transkribiert wurden.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF170/ Geschäftsprozess:}] Einbinden einer Login/Logout-Schaltfläche
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Ist der Benutzer nicht angemeldet, so ist eine ,,Login''-Schalt-fläche vorhanden, welche,
wenn sie geklickt wird, ein Dialogfeld öffnet in welcher der Benutzer sich nach \textbf{/LF200/} anmelden bzw. registrieren kann.
Nachdem der Benutzer angemeldet ist erscheint eine ,,Logout''-Schaltfläche die es dem Benutzer ermöglicht sich abzumelden.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF171/ Geschäftsprozess:}] Erstellen einer Profilseite
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Wenn der Benutzer angemeldet ist erscheint eine ,,Profile''-Schaltfläche in der Navigationsleiste.
Diese Seite beinhaltet allgemeine Daten über den Benutzer, unter anderem seit wann dieser angemeldet ist,
wie viele Objekte er schon bearbeitet hat sowie private Statistiken. Die Darstellung sollte sich nach \textbf{/NF171/} richten.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF200/ Geschäftsprozess:}] Registrieren und Anmelden
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Sofern der Benutzer nicht schon registriert ist, gelangt er durch das Dialogfeld in \textbf{/LF170} mit einem Klick
auf die ,,GitHub''-Schaltfläche auf die ,,Authorize application'' von GitHub um das Projekt zu autorisieren.
Nachdem der Benutzer dies getan hat, gelangt er auf die Willkommensseite von \textbf{/LF110} und ist angemeldet.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF300/ Geschäftsprozess:}] Darstellung von zu transkribierenden Objekten
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Das jeweilige Objekt soll initial in nativer Auflösung dargestellt werden und für den angemeldeten Nutzer interaktiv sein.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF310/ Geschäftsprozess:}] Skalierung und Bewegen des Objektes
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Durch das Scrollen oder Klicken auf die ,,+/-''-Schaltfläche kann der Benutzer in das Objekt hinein- bzw. herauszoomen.\\
Durch Drücken und Halten der linken Maustaste kann das Objekt frei bewegt werden.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF320/ Geschäftsprozess:}] Darstellung des vorherigen bzw. nachfolgenden Objektes
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Sofern ein Objekt ein nachfolgendes und / oder ein vorheriges Objekt besitzt,
so sollen diese in respektive links bzw. rechts von dem vom Benutzer ausgewählten Objektes liegen und sind als solche gekennzeichnet.
Damit diese ebenfalls transkribiert werden können.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF330/ Geschäftsprozess:}] Darstellung von ,,Areas of interest''
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Es sollen die Regionen, welche transkribiert werden bzw. wurden gekennzeichnet werden.
So kann der Benutzer mittels \textbf{/LF340/} neue Felder hinzufügen und diese transkribieren oder bereits existierende Felder betrachten,
diese auswählen und ebenfalls transkribieren.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF340/ Geschäftsprozess:}] Markieren von ,,Areas of interest''
\item[\textbf{Akteur:}] Benutzer
...
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF350/ Geschäftsprozess:}] Entfernen der letzten ,,Area of interest''
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Die Schaltfläche ,,R'' erlaubt es dem Nutzer, die letzte von ihm getätigte Markierung rückgängig zu machen.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF400/ Geschäftsprozess:}] Markdown Transkription
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Nachdem der Benutzer ein Bereich markiert hat bzw. einen bereits markierten Bereich ausgewählt hat, kann dieser diesen in einem großem,
sich selbst skalierendem Textfeld mittels Markdown transkribieren.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF410/ Geschäftsprozess:}] Syntax highlighting
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Die spezifische Markdown-Syntax soll hervorgehoben werden.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF420/ Geschäftsprozess:}] Vorschaufeld
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Der Benutzer soll die Möglichkeit haben, seine eingegebene Markdown-Transkription in einem Vorschaufeld zu betrachten.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/LF430/ Geschäftsprozess:}] Einbinden von Medieval Unicode Font Initiative
\item[\textbf{Akteur:}] Benutzer
\item[\textbf{Beschreibung:}] Mittels einer Schaltfläche kann der Benutzer ein Spezialzeichen aus der Medieval Unicode Font Initiative Datenbank auswählen,
um ein etwaiges spezielles Symbol in dem Objekt zu transkribieren.
\end{description}

\subsection*{Nichtfunktionale Anforderungen}
\begin{description}
\item[\textbf{/NF100/}] Selbsterklärende Benutzeroberfläche
\item[\textbf{Beschreibung:}] Die gesamte Webseite besitzt einen simplen, zugänglichen und vor allem selbsterklärenden Aufbau.
\\\rule{\linewidth}{0.4pt}
\item[\textbf{/NF171/}] Engagierender Aufbau des Projektes
\item[\textbf{Beschreibung:}] Damit der Benutzer ein Gefühl dafür erhält, wie viel er zu der allgemeinen Transkription eines Objektes beiträgt,
soll ein Punktesystem und ein eventuelles ,,Badges'' bzw. ,,Achievement'' System existieren. So soll es auch verschiedene Mechaniken geben,
welches die Benutzer zu einem eher kompetetiven Verhalten anspornt; unter anderem durch Bestenlisten.
\end{description}
...
In Abbildung \ref{fig:components} haben wir dafür die notwendigen Systeme, also
...
die für die Realisierung der TranscriptionDesk Website genutzt wurden.
...
Dies wird möglich, indem Vagrant eine virtuelle Maschine provisioniert\footnote{
    Es ließe sich mit Docker auch eine reine Containerlösung nutzen.
    \url{http://docker.com/}},
...
Danach planten wir die groben Entwicklungsetappen in drei Meilensteinen auf GitHub nach dem Modell der agilen Softwareentwicklung wie in Punkt 3.1 beschrieben.\\
...
ohne die jeweiligen Dateirechte zu verletzen und außerdem um die Algorithmik hinter dem System der Areas of Interest, welche der im Homer Multitext entwickelten Lösung ähneln sollte.
...
Diese Programmroutine erspart zudem das sichere Speichern von Passwörtern und sollte neben der Einrichtung der Omeka-API über den uns zur Verfügung gestellten Login eine der ersten umzusetzenden Funktionen sein.\\
...
In den folgenden Projektwochen konnten die Probleme mit Docker in der Vir-tualBox nicht gelöst werden, worauf wir nach kurzer Recherche mit einem Umstieg auf Vagrant reagierten.
...
\section*{Quellen}
  \printbibliography[%
    heading=none, % (none, bibintoc, bibnumbered)
  ]
\end{document}
